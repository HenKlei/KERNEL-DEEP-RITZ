...
from kernelDR.models.model_nn import NeuralNetworkModel, save_nn_model
...
from kernelDR.utils import (compute_relative_L2_error, compute_relative_H1_error, count_parameters,
                            peak_memory_mb, reset_peak_memory, set_seed)
...
def main(
    ...
    list_seeds: Annotated[tuple[int, ...], cyclopts.Parameter(help="RNG seeds to repeat the whole width sweep for. One result file is written per seed; aggregate them with aggregate_seed_runs.py.")] = (0,),
    save_models: Annotated[bool, cyclopts.Parameter(help="Save the trained network of every (width, seed), so that its error metrics can be recomputed later on a different evaluation grid without repeating the training.")] = False,
    results_dir: Annotated[str, cyclopts.Parameter(help="Results output directory.")] = "results_neural_network_singular_solution/",
):
    ...
    seed_tag = "-".join(str(s) for s in list_seeds)
    save_settings(results_dir, locals(), filename=f"settings_seed{seed_tag}.txt")
    ...
    for seed in list_seeds:
        results_file = results_dir + f"convergence_results_seed{seed}.txt"
        ...
        for idx_n, (n, n_ep) in enumerate(zip(list_num_neurons_per_layer, list_n_epochs)):
            # Re-seed per width, so that a single (width, seed) run reproduces the
            # corresponding row of the full sweep. Across widths this keeps the
            # random streams paired, across seeds it makes the runs independent.
            set_seed(seed)

            model = NeuralNetworkModel(problem.domain.dim, problem.output_dim, n, num_layers, activation=activation).to(device)

            num_parameters = count_parameters(model)
            print(f"Seed {seed}, number of parameters: {num_parameters}")
            ...
            print(datetime.now().strftime("%H:%M:%S"),
                  'Computation {}/{} for seed {} finished.'.format(idx_n + 1, len(list_num_neurons_per_layer), seed))
            ...
            if save_models:
                model_path = results_dir + f"model_seed{seed}_width{n}.pt"
                save_nn_model(model, model_path)
